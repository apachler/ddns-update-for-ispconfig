...
// True to allow local tests with PHP-CGI from cli
$debug = true;
if ($debug) {
...
$soap_location = 'https://example.com:8080/remote/index.php';
$soap_uri      = 'https://example.com:8080/remote/';

// Exception list for $myip (force to use $_SERVER["REMOTE_ADDR"])
$exception = array(
    'example.com',
    'example2.com.'
);
...
// Check if the remote user is allowed to update the passed host
$json = file_get_contents('./.allowedhosts.json');
$jsondata = json_decode($json, true);
$allowedhosts = isset($jsondata[$_SERVER['PHP_AUTH_USER']]) ? $jsondata[$_SERVER['PHP_AUTH_USER']] : false;
if ($allowedhosts && !in_array($_GET["hostname"], $allowedhosts)) {
    $log->debug("Remote user is not allowed to change the hostname " . $_GET["hostname"]);
    echo "dnserr";
    exit;
} else {
    $log->debug("Ok, update of hostname " . $_GET["hostname"] . " is allowed!");
}
...
try {
    if ($session_id = $client->login($_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW'])) {
        $log->debug('Login successfull (Session ID: ' . $session_id . ')');
    } else {
        $log->debug('Login NOT successfull (Session ID: ' . $session_id . ')');
        echo "badauth";
        exit;
    }
    ...
    $zone = $client->dns_zone_get($session_id, array(
        'origin' => $dnszone
    ));
    if (count($zone) == 0) {
...
//        $affected_rows = 0;
        ...
        // Load DNS host record
        $hostname = substr($_GET["hostname"], 0, $firstdotpos);
        $log->debug("Load DNS host record for given zone: " . $hostname);

        // if IPv4 use dns_a_get, if ipv6 use dns_aaaa_get
        if ($is_ipv4) {
            $dns_record = $client->dns_a_get($session_id, array(
                'zone' => $zone[0]['id'],
                'name' => $hostname
            ));
        } else if ($is_ipv6) {
            $dns_record = $client->dns_aaaa_get($session_id, array(
                'zone' => $zone[0]['id'],
                'name' => $hostname
            ));
        }

        // check if we got a record at all
        if (count($dns_record) == 0) {
            $log->debug("DNS host record not found at " . time());
            echo "dnserr";
            exit;
        }

        $log->debug("DNS host record found");

        // check if record is an A type (not cname, mx, txt, ...)
        ...
        if ($is_ipv4 && $dns_record[0]['type'] != 'A') {
            ...
            echo "dnserr";
            exit;
        } else if ($is_ipv6 && $dns_record[0]['type'] != 'AAAA') {
            $log->debug("Record type is not AAAA. You can only update a AAAA record with IPv6");
            echo "dnserr";
            exit;
        }

        $log->debug("Check record type successfull");

        if ($dns_record[0]['data'] != $ip) {
            $dns_record[0]['data'] = $ip;

            // if IPv4 use dns_a_update, if ipv6 use dns_aaaa_update
            if ($is_ipv4) {
                $affected_rows = $client->dns_a_update($session_id, 0, $dns_record[0]['id'], $dns_record[0]);
            } else if ($is_ipv6) {
                $affected_rows = $client->dns_aaaa_update($session_id, 0, $dns_record[0]['id'], $dns_record[0]);
            }
